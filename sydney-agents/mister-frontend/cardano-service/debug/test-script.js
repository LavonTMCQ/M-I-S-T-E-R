// Test script address generation separately
import { serializePlutusScript } from '@meshsdk/core';
import { applyParamsToScript } from '@meshsdk/core-csl';

console.log('Testing script address generation...');

// Use the hello_world validator from Aiken blueprint
const HELLO_WORLD_BLUEPRINT = {
  "validators": [
    {
      "title": "hello_world.hello_world.spend",
      "compiledCode": "",
      "hash": "167f56e1b5de377df88962340a0461158e68d4b6caaea9d27c9d71e5"
    }
  ]
};

try {
  console.log('1. Testing applyParamsToScript...');
  const scriptCbor = applyParamsToScript(
    HELLO_WORLD_BLUEPRINT.validators[0].compiledCode,
    []
  );
  console.log('✅ applyParamsToScript successful');
  console.log('Script CBOR length:', scriptCbor.length);
  
  console.log('2. Testing serializePlutusScript for mainnet...');
  const scriptAddr = serializePlutusScript(
    { code: scriptCbor, version: "V3" },
    1 // mainnet
  ).address;
  console.log('✅ serializePlutusScript successful');
  console.log('Script address:', scriptAddr);
  
} catch (error) {
  console.error('❌ Error:', error.message);
  console.error('Stack:', error.stack);
}
...
export class PineScriptKnowledgeBase {
  ...
  async searchKnowledge(query: string, limit = 10) {
    ...
    // Use memory search if available, otherwise return empty results
    if ('search' in this.memory && typeof this.memory.search === 'function') {
      return await (this.memory as any).search(query, {
        limit,
        scope: 'resource',
      });
    }
  ...
  async addKnowledge(_content: string, metadata: any) {

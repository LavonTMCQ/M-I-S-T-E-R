...
export const macdHistogramBacktest = createTool({
  ...
  execute: async (context) => {
    try {
      ...
      const _startMonth = `${start.getFullYear()}-${String(start.getMonth() + 1).padStart(2, '0')}`;
      const _endMonth = `${end.getFullYear()}-${String(end.getMonth() + 1).padStart(2, '0')}`;
      ...
      const backtestResult = {
        ...
        performance: {
          ...
          largestLoss: losingTrades.length > 0 ? Math.abs(Math.min(...losingTrades.map(t => t.pnl || 0))) : 0
        },
        trades: trades.slice(0, 10).map(trade => ({
          id: `trade-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
          symbol,
          type: trade.side === 'long' ? 'BUY' as const : 'SELL' as const,
          ...
          reason: 'MACD signal'
        })), // Store first 10 trades as examples
        dataQuality: {
          priceDataPoints: priceData.length,
          macdDataPoints: macdData.length,

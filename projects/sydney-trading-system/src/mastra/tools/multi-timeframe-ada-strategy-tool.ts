  ...
  execute: async ({ context }) => {
    ...
    try {
      ...
      for (const tf of timeframes) {
        ...
        const data = await fetchKrakenData(symbol, tf, startDate, endDate);
        marketData[tf] = data;
      }
...
async function fetchKrakenData(symbol: string, timeframe: string, startDate: string, endDate: string): Promise<TimeframeData[]> {
  const url = `https://api.kraken.com/0/public/OHLC?pair=${symbol}&interval=${timeframe}`;
  ...
  return data.result[ohlcKey].map((candle: any[]) => ({
    timestamp: candle[0],
...
function generateStrategyAnalysis(results: any, marketData: any, indicators: any) {
  ...
  return {
    ...
    riskMetrics: {
      maxRiskPerTrade: trades.length > 0 ? Math.max(...trades.map(t => Math.abs(t.pnl))) : 0,
      avgRiskPerTrade: trades.length > 0 ? trades.reduce((sum, t) => sum + Math.abs(t.pnl), 0) / trades.length : 0,
...
function calculateRiskMetrics(results: any, initialCapital: number) {
  ...
  return {
    ...
    avgWin: trades.length > 0 ? trades.filter(t => t.pnl > 0).reduce((sum, t) => sum + t.pnl, 0) / Math.max(1, trades.filter(t => t.pnl > 0).length) : 0,
    avgLoss: trades.length > 0 ? trades.filter(t => t.pnl < 0).reduce((sum, t) => sum + t.pnl, 0) / Math.max(1, trades.filter(t => t.pnl < 0).length) : 0,

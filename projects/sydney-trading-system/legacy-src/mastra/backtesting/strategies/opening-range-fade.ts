...
export class OpeningRangeFadeStrategy extends BaseStrategy {
  ...
  readonly description = 'Fades breakouts from the opening range - counter-trend strategy optimized for mean reversion (v2)';
  ...
  readonly parameters: OpeningRangeFadeParameters = {
    ...
    fadeThreshold: 0.002,          // 0.2% - same as original breakout threshold
    volumeMultiplier: 1.2,         // Same volume confirmation
    stopLossATRMultiplier: 2.0,    // Wider stops for counter-trend
    takeProfitRatio: 1.5,          // Tighter targets for mean reversion
    maxPositionTime: 120,          // 2 hours - faster exits
    minRangeSize: 0.0005,          // 0.05% - same as original
    exitBeforeClose: 30
  };
  ...
  private shouldExit(context: StrategyContext, state: StrategyState): StrategySignal | null {
    ...
    // Exit before market close (simple approach: 4:00 PM)
    const marketClose = new Date(currentTime);
    marketClose.setHours(16, 0, 0, 0);

    const exitTime = new Date(marketClose);
    exitTime.setMinutes(exitTime.getMinutes() - this.parameters.exitBeforeClose);
    ...
    if (currentTime >= exitTime) {
      return {
        ...
        reason: `Exit before market close (${this.parameters.exitBeforeClose} minutes before)`,
        confidence: 0.9,
        timestamp: currentBar.timestamp,
        metadata: { exitReason: 'market-close' }
      };
    }

    // Exit after maximum position time
    const positionDuration = currentTime.getTime() - new Date(position.entryTime).getTime();
    const maxDurationMs = this.parameters.maxPositionTime * 60 * 1000;

    if (positionDuration >= maxDurationMs) {
      return {
        type: position.type === 'LONG' ? 'SELL' : 'BUY',
        strength: 'STRONG',
        price: currentBar.close,
        reason: `Maximum position time reached (${this.parameters.maxPositionTime} minutes)`,
        confidence: 0.8,
        timestamp: currentBar.timestamp,
        metadata: { exitReason: 'max-time' }
      };
    }
  ...
  private createFadeShortSignal(currentBar: OHLVC, context: StrategyContext): StrategySignal {
  ...
  private createFadeLongSignal(currentBar: OHLVC, context: StrategyContext): StrategySignal {
    const entryPrice = currentBar.close;
  ...
  private updateTechnicalIndicators(currentBar: OHLVC, context: StrategyContext): void {
    ...
    // Update ATR
    if (context.previousBar) {
      const tr1 = currentBar.high - currentBar.low;
      const tr2 = Math.abs(currentBar.high - context.previousBar.close);
      const tr3 = Math.abs(currentBar.low - context.previousBar.close);
  ...
  private isNewTradingDay(currentTime: Date, state: StrategyState): boolean {
    if (!state.lastProcessedTime) return true;
    
    const lastDate = new Date(state.lastProcessedTime);
